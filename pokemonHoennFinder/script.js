...
function fetchHoennPokemon() {
    ...
    fetch('http://localhost:3000/hoenn')
        .then((response) => response.json())
        .then((json) => {
            json.data.url_data.forEach(item => {
...
function fetchPokemonData(url) {
    fetch(`http://localhost:3000/fetchPokemon?url=${url}`)
        .then(response => response.json())
        .then((json) => {
            renderPokemon(json.data.name, json.data.id, json.data.types)
        })
}

function renderPokemon(name, id, types) {

    const template = document.getElementById("card-template").content.cloneNode(true);
    template.querySelector('.card-img-top').src = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${id}.png`;
    template.querySelector('.pokemonName').innerText = Capitalize(name);
    template.querySelector('.pokemonNumber').innerText = `#${id}`;
    let pokeTypes = template.querySelector('.pokemonTypes');
    createTypes(types, pokeTypes)
...
function initSearchableArray() {
    ...
    fetch('http://localhost:3000/hoenn')
        ...
        .then((json) => {
            ...
            json.data.pokemon_name_data.forEach(item => {
...
function initDropdown() {
    ...
    fetch('http://localhost:3000/hoenn')
        ...
        .then((json) => {
            json.data.url_data.forEach(item => {
                fillDropdown(json.data.pokemon_name_data[i], item, form);
                i++;
            })
        });
...
function renderCounters(typeCounter) {
    ...
    searchable[1].forEach(item => {
        fetch(`http://localhost:3000/fetchPokemon?url=${item}`)
            .then(response => response.json())
            .then(json => {
